...
class GateController extends Controller
{
    ...
    private function isShoeReturn($barcodeId, &$errorList)
    {
        ...
        if (is_null($skateTransaction->rent_start)) {
            ...
            // STATUS: RENT
            $gsGateModeType = GeneralSetting::all()->first()->gate_mode;
            if (GateModeType::gateFirstRentSecond()->isEqual($gsGateModeType)) {
                // strict mode, customer cannot pass gate before return
                array_push($errorList, 'Skate belum dikembalikan');
                return false;
            } else if (GateModeType::rentFirstGateSecond()->isEqual($gsGateModeType)) {
                // allow customer to go out and return the skate
                return true;
            }
        } else {
    ...
    {
        // TODO: Skating aid tidak memiliki barcode_id yg dapat digunakan untuk mengecek customer sewaktu keluar dari gate
        // TODO: Setelah diskusi, kami tidak akan mengimplementasikannya
